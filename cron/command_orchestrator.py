...
# - Fixes auth policy: when both secrets exist, accept EITHER scheduler header OR HMAC signature
# - Keeps payload schema, endpoints, idempotency, SLA, ToolCatalog behavior unchanged
# - No breaking changes
...
# Simple scheduler secret (for Render Cron headers)
...
def verify_request_auth_or_401(raw_body: bytes, headers: Dict[str, str]) -> None:
    ...
    - If both SCHEDULER_SECRET and RUN_SHARED_SECRET are set:
        accept EITHER x-scheduler-secret OR x-run-signature.
    - If only SCHEDULER_SECRET is set:
        require x-scheduler-secret.
    - If only RUN_SHARED_SECRET is set:
        require x-run-signature.
    - Else: no auth required (dev mode).
    """
    scheduler_enabled = bool(SCHEDULER_SECRET)
    hmac_enabled = bool(RUN_SHARED_SECRET)

    got_sched = (headers.get("x-scheduler-secret") or "").strip()
    got_sig = (headers.get("x-run-signature") or "").strip()

    if scheduler_enabled and hmac_enabled:
        if got_sched:
            _verify_scheduler_secret_or_401(headers)
            return
        if got_sig:
            _verify_hmac_signature_or_401(raw_body, got_sig)
            return
        raise HTTPException(status_code=401, detail="Missing auth (x-scheduler-secret or x-run-signature)")
...
async def run(request: Request, response: Response) -> RunResponse:
    ...
    # Auth (SAFE): scheduler secret OR HMAC signature depending on envs
