...
def capability_incident_router(req: RunRequest, run_record_id: str) -> Dict[str, Any]:
    ...
    reason = str(payload.get("reason") or "unknown_incident")
    failed_url = str(payload.get("failed_url") or "")
    http_status = payload.get("http_status")

    decision = "incident_created"

    next_commands = [
        # 1. Créer incident (HTTP mock Airtable)
        {
            "capability": "http_exec",
            "priority": 3,
            "input": {
        ...
        # 2. Alert (Slack / webhook)
        {
            ...
            "input": {
                ...
                "goal": "alert_incident",
                "body": {
                    "text": f"INCIDENT: {reason} | status={http_status}",
                    "flow_id": flow_id,
                },
            },
        },
